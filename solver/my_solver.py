...
def solve(data_folder: str) -> int:
    ...
    for seed_i in order:
        ...
        fx = np.clip(comp_tl_x[seed_i] + fov_w / 2, fov_w / 2, pcb_w - fov_w / 2)
        fy = np.clip(comp_tl_y[seed_i] + fov_h / 2, fov_h / 2, pcb_h - fov_h / 2)
    ...
    for i in range(n):
        ...
        if n_x == 1 and n_y == 1:
            ...
            for gx in range(n_x):
                for gy in range(n_y):
                    fx = comp_tl_x[i] + fov_w / 2 + gx * fov_w
                    fy = comp_tl_y[i] + fov_h / 2 + gy * fov_h
                    fx = min(fx, comp_br_x[i] - fov_w / 2)
                    fy = min(fy, comp_br_y[i] - fov_h / 2)
                    fovs.append({'x': fx, 'y': fy, 'comp_idx': [i], 'step': comp_step[i]})
        assigned[i] = True
